    ...
    <div className={cn(pricingColumnVariants({ variant, className }))} {...props}>
      ...
      <div className="flex flex-col gap-7">
        <header className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground max-w-[240px] text-sm">{description}</p>
        </header>
        <section className="flex flex-col gap-3">
          {originalPrice !== undefined && (
            <div className="flex h-6 items-baseline gap-1">
              <span className="text-muted-foreground text-lg font-medium line-through">
                {originalPrice > 0 && price !== originalPrice ? `$${originalPrice}` : ''}
              </span>
            </div>
          )}
          <div className="flex items-center gap-3">
            <div className="flex flex-col gap-1">
              <div className="flex items-baseline gap-1">
                ...
                <span className={cn(isNumericPrice ? 'text-5xl' : 'text-5xl', 'font-bold')}>
                  {price}
                </span>
              </div>
            </div>
          ...
          {promotionText && (
            <div className="text-brand-foreground h-6 text-sm font-medium">{promotionText}</div>
          )}
        </section>
        ...
        <div className="text-muted-foreground min-h-[45px] max-w-[220px] text-sm leading-6">
          {priceSubline && <div className="">{priceSubline}</div>}
          {priceNote}
        </div>
